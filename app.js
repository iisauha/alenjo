...
// Hard-reload so iOS replays its AutoFill heuristics and pops the
// native Face ID / password sheet, same as a fresh app open.
...
async function removeCardBackground(blob) {
  ...
  var result = await mod.removeBackground(blob, {
    model: 'isnet_quint8',
...
$('#card-crop-apply').addEventListener('click', async function() {
  ...
  var extractedBlob;
  try {
    setDesignStatus(bgRemovalEverLoaded ? 'Extracting card...' : 'Loading extraction model (first time, ~40MB)...');
...
function cacheRefineCanvasRect() {
  var c = refineState.displayCanvas;
  if (c) refineState.canvasRect = c.getBoundingClientRect();
}
...
(function wireRefineEvents() {
  ...
  window.addEventListener('resize', function() { refineState.canvasRect = null; });
})();
  ...
  if (summaryEl) {
    ...
    if (totalExpenses > 0 || totalIncome > 0) {
      ...
      if (surplus < 0) {
        ...
        summaryHtml += checkingLabel + ' in checking';
        if (creditOwed > 0) summaryHtml += ', ' + formatMoney(creditOwed) + ' owed on cards';
        summaryHtml += '. ' + formatMoney(totalExpenses) + ' in bills over the next ' + horizonLabel;
        if (totalIncome > 0) summaryHtml += ' with ' + formatMoney(totalIncome) + ' coming in';
        summaryHtml += '. Things may get tight. Consider setting aside a buffer.';
        summaryHtml += '</div>';
        ...
        summaryHtml += checkingLabel + ' in checking';
        if (creditOwed > 0) summaryHtml += ', ' + formatMoney(creditOwed) + ' owed on cards';
        summaryHtml += '. Covered for ' + formatMoney(totalExpenses) + ' in bills over the next ' + horizonLabel;
        if (totalIncome > 0) summaryHtml += ' plus ' + formatMoney(totalIncome) + ' income';
        summaryHtml += '.';
        summaryHtml += '</div>';
        summaryHtml += '</div>';
      }
